    ...
    if "UNK" == intent_pred:
        ...
        if ent_label and ent_name and ent_pro_key:
            if len(ent_pro_key)<2:
                ...
                if '技术特征' in answer.keys():
                    answer['技术特征'] = '设备名称' + ent_name + ', ' + answer['技术特征']
                answer = json.dumps(answer, indent=2, ensure_ascii=False) + "\n" + tips
            else:
                answer = queryEntityPro_multi(ent_label,ent_name,ent_pro_key)
                answer = json.dumps(answer, indent=2, ensure_ascii=False) + "\n" + "--Tips:查询单实体多个属性值"
            
            return answer,ent_name
        else:
        ...
        for key,value in zip(slot_key,slot_value):
            ...
            if 'pro.' in key:
                if key in property_dict:
                    pro_ls.append(property_dict[key])
                else:
                    key_error = 'slot_key error: slot_key not in property_dict'
                    return tips + "\n" + key_error
        answer = queryEntityAllPro_multi(ent_label_ls,ent_name_ls,pro_ls)
        answer = json.dumps(answer,indent=2,ensure_ascii=False) + "\n" + tips
        print('answer:',answer)
        return answer,ent_name
        ...
        if ent_label and ent_name and rel:
            if rel == '责任人':
                ...
                answer = queryRelEntity(label,ent,rel) 
            ...
            return tips + '\n' + queryParam_error
    elif "query_related_entity_two" == intent_pred:
        ...
        if not rel_ls:
            ...
            rel_ls.append(rel)
        if ent_label and ent_name and rel:
            if len(rel_ls) < 2 and ('生产厂家' not in rel_ls) and ('责任人' not in rel_ls) and ('安装地点' not in rel_ls):
                answer = queryRelEntity(ent_lab,ent,rel_ls[0])
                answer = json.dumps(answer,indent=2,ensure_ascii=False)+ "\n" + "--Tips:查询 实体-关系->实体(单个关系)"
...
# 2.2 查询多实体属性（全部属性值）
def queryEntityAllPro_multi(label_ls,name_ls,pro_ls):
    '''
    label:实体label
    name_ls:实体名称列表
    '''
...
def queryRelEntity(label, name, rel):
    ...
    # 如果是实体——关系，就是下面这样
    if len(label) == 1:
        cypher = "MATCH (n:`%s`{name:'%s'})-[r:`%s`]->(m) RETURN m"%(label[0],name[0],rel)
    # 如果是实体——关系——实体，就是要加一个实体，ent是数组
    else:
        # 因为矿山——设备不是直接有关系的，这么问时会查库失败，所以自己建立一个直接的关系“所属煤矿是xx矿山的设备”，方便查询
        if label[0] == '矿山' and label[1] == '设备':
            cypher = "MATCH (d:%s) WHERE d.所属矿山 = '%s' RETURN d.name"%(label[1],name[0])
            result = pd.DataFrame(graph.run(cypher))
            print(result[0])
            ...
            cypher = "MATCH (n:`%s`{name:'%s'})-[r:`%s`]->(m:`%s`) RETURN m"%(label[0],name[0],rel,label[1])
    time.sleep(3)  # 在连接之前延时3秒
    result = pd.DataFrame(graph.run(cypher))
    if result.empty:
...
def query_related_entity_two(label,name,rel_ls):
    ...
    if not rel_ls:
        return '关系类别(rel)非法'
            ...
            if len(label)<3:
                # "MATCH (n:`企业`{name:'淮南煤矿机械厂'})-[:`制造`]->(m:`设备`{name:'JM-28慢速绞车'})-[:`安装地点`]->(k) RETURN k.name"
                cypher = "MATCH (n:`%s`{name:'%s'})-[:`%s`]->(m:`%s`{name:'%s'})-[:`%s`]->(k) RETURN k.name"%(label[0],name[0],rel_ls[0],label[1],name[1],rel_ls[1])
                # cypher = "MATCH (n:%s)-[:%s]->(m:%s)-[:%s]->(k) WHERE n.name = '%s' RETURN [m.name,k.name]"%(label[0],rel_ls[0],label[1],label[2],name[0])
            else:
                # "MATCH (n:`分公司`{name:'华亭煤业公司'})-[:`包含`]->(m:`矿山`{name:'华亭煤矿'})-[:`包含`]->(k:`设备类型`) RETURN k.name"
                cypher = "MATCH (n:`%s`{name:'%s'})-[:`%s`]->(m:`%s`{name:'%s'})-[:`%s`]->(k:`%s`) RETURN k.name"%(label[0],name[0],rel_ls[0],label[1],name[1],rel_ls[1],name[2])
